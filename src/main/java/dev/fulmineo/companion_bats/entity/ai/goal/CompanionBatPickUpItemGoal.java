...
public class CompanionBatPickUpItemGoal extends Goal {
    ...
    private boolean isBundleAvailable(){
        return this.bundleStack != null && this.bundleStack.isOf(Items.BUNDLE) && (BundleItemInvoker.invokeGetBundleOccupancy(this.bundleStack) / 64) != 1;
    }

    private boolean canItemFitInBundle(ItemStack itemStack) {
        return (BundleItemInvoker.invokeGetBundleOccupancy(this.bundleStack) / 64) + (BundleItemInvoker.invokeGetItemOccupancy(itemStack) / 64 * itemStack.getCount()) <= 1;
    }
